...
Record = namedtuple("Team", "code logo wins losses gb")
...
def standings(date: date, league: str = 'AL') -> Standings:
    ...
    for g in gamelog:
        if g.league == league and g.date <= date:
            ...
            wins.update({loser: wins.get(loser, 0) + 0})
            losses.update({loser: losses.get(loser, 0) + 1})
            losses.update({winner: losses.get(winner, 0) + 0})
    leader = max(wins, key=lambda k:wins[k]/(wins[k]+losses[k]))
    unsorted_standings = [Record(team_name(team), team_logo(team),wins[team], losses[team], gb(team, leader)) for team in wins]
